...
- `finding.poc.payload_contract`: where the payload belongs and what the sink
  should observe.
...
Use `agent_packet` as the default input to coding agents. Treat the rest of
`finding.poc` and the full `report.json` as an evidence store that should be
loaded only when the compact packet is insufficient or validation fails.
...
- If needed, read selected evidence from `finding.poc.thin_slice`,
  `finding.poc.trace`, `finding.path`, and `finding.rule_evaluation`.
...
This is a prompt/template solution. It does not own a durable orchestration
loop, retry policy, multi-agent backend selection, or benchmark-grade run logs.

## Solution B: Automated Runner Workflow

Location: `pocgen/`

This solution is a normal Python implementation, similar in shape to BugAtlas:
prompts plus a CLI runner that can call a headless coding agent such as Codex,
...
This is an executable pipeline solution. It should not be hidden inside the
skill because it needs durable state, batch controls, backend selection,
validation, retries, and result accounting.
...
| --- | --- | --- |
| Primary user | Human-assisted Codex session | Batch/reproducible pipeline |
| Implementation | Skill instructions and templates | Python CLI plus prompts |
| Default input | `finding.poc.agent_packet` | `finding.poc.agent_packet` |
| Evidence loading | Agent reads on demand | Script controls staged evidence |
| Validation | Manual or agent-driven | Automated, logged, retryable |
| Best for | One-off PoC work | Benchmarking and large-scale PoC generation |

The skill should document how to reason about a finding. The automated runner
should implement the repeatable process.
